...
start_a = 1
while True:
    reg = {"a": start_a, "b": 0, "c": 0, "d": 0, "out": None}
    # print("trying:", start_a)
    pos = 0
    steps = 0
    stat = [0] * len(program)
    bad = False
    while not bad and 0 <= pos < len(program):
        steps += 1
        stat[pos] += 1
        if steps % 100000 == 0:
            print(steps, stat)
        ...
        if steps > 1000000:
            print("Went 1000000 steps without a failure, this is probably gonna work")
            found = True
            break
    if found:
        break
